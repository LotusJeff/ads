...
class acp_ads_module
{
	...
	{
		global $db, $table_prefix, $user, $auth, $request, $template;
		...
		//$user->add_lang_ext('bb3mobi/ads', 'ads');
		$this->tpl_name = 'acp_ads';
		$this->page_title = 'ACP_ADVERTISEMENT_MANAGEMENT';

		$action = $request->variable('action', '');
		$submit = $request->is_set_post('submit');
		$position_id = $request->variable('p', 0);
		$ad_id = $request->variable('a', 0);
		$error = array();
		...
		$this->new_config = ($request->is_set('config')) ? utf8_normalize_nfc($request->variable('config', array('' => ''), true)) : $this->new_config;

		// Other settings
		$ad_name = utf8_normalize_nfc($request->variable('ad_name', '', true));
		$ad_code = utf8_normalize_nfc($request->variable('ad_code', '', true));
		$ad_note = utf8_normalize_nfc($request->variable('ad_note', '', true));
		$ad_time_end = (utf8_normalize_nfc($request->variable('ad_time_end', '', true))) ? strtotime(utf8_normalize_nfc($request->variable('ad_time_end', '', true))) : 0;
		$ad_groups = $request->variable('ad_groups', array(0), true);
				...
				else if ($ad_id || !$position_name)
				{
					...
					if ($submit && !sizeof($error))
					{
						$sql_ary = array(
							...
							'ad_views'			=> $request->variable('ad_views', 0),
							'ad_view_limit'		=> $request->variable('ad_view_limit', 0),
							'ad_clicks'			=> $request->variable('ad_clicks', 0),
							'ad_click_limit'	=> $request->variable('ad_click_limit', 0),
							'ad_priority'		=> $request->variable('ad_priority', 5),
					...
					else
					{
						...
						$template->assign_vars(array(
							...
							'AD_VIEW_LIMIT'		=> (($action == 'edit' || $action == 'copy') && !$submit) ? $ad_data['ad_view_limit'] : $request->variable('ad_view_limit', 0),
							'AD_VIEWS'			=> (($action == 'edit' || $action == 'copy') && !$submit) ? $ad_data['ad_views'] : $request->variable('ad_views', 0),
							'AD_CLICK_LIMIT'	=> (($action == 'edit' || $action == 'copy') && !$submit) ? $ad_data['ad_click_limit'] : $request->variable('ad_click_limit', 0),
							'AD_CLICKS'			=> (($action == 'edit' || $action == 'copy') && !$submit) ? $ad_data['ad_clicks'] : $request->variable('ad_clicks', 0),
							'AD_PRIORITY'		=> (($action == 'edit' || $action == 'copy') && !$submit) ? $ad_data['ad_priority'] : $request->variable('ad_priority', 5),
